		...
		return this.each(function () {
			...
			if (opts.dir == "v") {
				...
				if (_this.get(0).scrollHeight <= _oheight) {
					...
					// 获取内容，分配到一个新的容器
					var _scroll_zone = $('<div class="scroll_zone"></div>').html(_ocontent).css({
					...
					// 清空原有的容器,用的容器进行填充
					_this.empty().append(_scroll_zone).css({
						position : 'relative',
						'overflow' : 'hidden'
